...
class Controller(nn.Module):
    def __init__(self, in_features=80, out_features=3):
        """
...
class hDQN():
    ...
    def __init__(self,
        ...
        self.meta_controller = MetaController(in_features=79, out_features=7).type(dtype)
        self.target_meta_controller = MetaController(in_features=79, out_features=7).type(dtype)
    ...
    def is_goal_reached(self, goal, obs):
        if goal == 6:
            return np.any(obs["image"][:, :, 0] == 6)  # 是否有绿色箭头格子
        return False
    ...
    def update_meta_controller(self, gamma=1.0):
        ...
        # 关键：reshape state_batch 变成 (batch_size, feature_dim)
        # 这里 feature_dim 就是 fc1 的输入维度，比如 79
