...
class hDQN():
    ...
    def _goal_1(self, obs):  # 开门
        door = next((obj for obj in self.env.unwrapped.grid.grid if obj and obj.type == 'door'), None)
        result = door.is_open if door else False
        if result:
            self.completed_goals.add(1)
        return result

    def _goal_2(self, obs):  # 到达终点
        agent_pos = self.get_agent_position()
        cell = self.env.unwrapped.grid.get(agent_pos[0], agent_pos[1])
        result = cell is not None and cell.type == 'goal'
        if result:
            self.completed_goals.add(2)
        return result

    def select_goal(self, state, epsilon):
        available_goals = [g for g in range(self.num_goal) if g not in self.completed_goals]
        ...
        if sample > epsilon:
            ...
            # 只在 available_goals 中选择最大 Q 的目标
            best_goal = max(available_goals, key=lambda g: q_values[g].item())
            return best_goal
        else:
            # 随机探索一个未完成的目标
    ...
    #         next_goal = max(self.completed_goals) + 1
    #     else:
    #         next_goal = 0
    #
    #     # 如果全部完成了，直接返回 None 或者 -1
    #     if next_goal >= self.num_goal:
    #         return None
    #
    #     sample = random.random()
    #     if sample > epsilon:
    ...
    #             q_values = self.meta_controller(state.unsqueeze(0))[0]
    #
    #         # 直接返回 next_goal，或者检查 q_values[next_goal]
    #         return next_goal
    #     else:
    #         # 探索时，也只在 next_goal 里选（不随机选多个）
    #         return next_goal
    ...
    def get_meta_reward(self, goal, goal_reached):
        if not goal_reached:
            return 0.0
        goal_rewards = {
            0: 0.2,
        ...
        return goal_rewards.get(goal, 0.0)
